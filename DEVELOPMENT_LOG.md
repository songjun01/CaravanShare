...
- **작업 내용**: 프로필 수정 페이지에서 프로필 이미지 등록 및 변경 기능
- **Gemini CLI 사용 프롬프트**:
```
프로필 수정 페이지에서 프로필 이미지를 등록하고 변경할 수 있게 해줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 이미지 업로드 방법
...
- **작업 내용**: 게스트와 호스트 명시적 구분
- **Gemini CLI 사용 프롬프트**:
```
호스트와 게스트를 명시적으로 구분하고 싶어. 프로필 페이지에 게스트나 호스트라는걸 보여주는 칸을 만들어줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 관리
...
- **Gemini CLI 사용 프롬프트**:
```
카라반의 상태를 관리하는 기능을 만들거야. 호스트는 자신이 등록한 카라반의 상태를 관리할 수 있는데, [사용가능, 예약됨, 정비중] 이라는 메시지를 띄울 수 있어. 라디오 버튼으로 선택하게 하고 싶어. 카라반 수정 페이지에서 관리할 수 있고, 카라반 상태를 게스트가 caravans 페이지에서 볼 수 있어.
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 데이터베이스-백엔드-프론트엔드 통신 방법

---

### Day 7 (2025.11.21)

- **작업 내용**: 게스트 로그인 시 '내 카라반' 버튼 숨기기
- **Gemini CLI 사용 프롬프트**:
```
현재 게스트로 로그인 했는데 우측 상단에 내 카라반 버튼이 활성화 되어있어. 카라반을 추가하는 기능은 호스트에게만 있는거잖아. 게스트로 로그인 했을 경우에는 이 버튼을 없애줘.
```
- **결과 및 수정사항**: 구현 성공 (client/src/components/Header.jsx 수정)
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 UI 조건부 렌더링

---

- **작업 내용**: 카라반 상태 관리 기능 개선
- **Gemini CLI 사용 프롬프트**:
```
호스트로 로그인 했을 때 내 카라반을 수정하는 창에서 카라반의 상태를 관리하는 부분을 만들거야. 카라반의 상태는 [사용가능, 예약됨, 정비중]으로 이루어져 있고, 이 정보가 메인 페이지의 각 카라반 박스의 이름 우측에 나타나고, 카라반 상세 페이지에서 사용가능일 경우에만 예약하기 버튼이 나오고, 예약됨이나 정비중 상태인 경우 이 텍스트가 버튼자리에 나오게 해줘
```
- **결과 및 수정사항**: 구현 성공 (server/src/models/caravan.model.js, client/src/pages/EditCaravanPage.jsx, client/src/components/CaravanCard.jsx, client/src/pages/CaravanDetailPage.jsx 수정)
- **학습 내용**: 카라반 상태 관리를 위한 백엔드 모델 업데이트 및 프론트엔드 UI 조건부 렌더링 구현
...
- **Gemini CLI 사용 프롬프트**:
```
게스트로 로그인했을 때 예약을 신청하는 기능을 추가해줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가

---

- **작업 내용**: 예약 승인/거절 기능 추가
- **Gemini CLI 사용 프롬프트**:
```
호스트로 로그인 했을 때 예약 승인/거절 기능을 추가해줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **작업 내용**: 예약 날짜 관리 (캘린더) 기능 추가
- **Gemini CLI 사용 프롬프트**:
```
예약 날짜 관리 (캘린더) 기능을 구현해줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **Gemini CLI 사용 프롬프트**:
```
게스트가 예약한 카라반의 내역을 볼 수 있게 목록으로 보여주는 페이지를 만들어줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **Gemini CLI 사용 프롬프트**:
```
결제 및 가격에서 선결제 시스템을 구현해줘. 예약 승인이 되었을 경우 결제를 진행할 수 있게 해줘.
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **작업 내용**: 거래 후 리뷰 작성 기능 추가
- **Gemini CLI 사용 프롬프트**:
```
리뷰/평가에서 거래 후 리뷰 작성 기능을 구현해줘. 예약 승인과 결제가 모두 완료되었을 때, 내 예약 목록에서 해당 카라반 바로 하단에 블럭이 생기고 블럭 가장 우측에 리뷰를 작성할 수 있는 버튼이 나올거야. 이 버튼을 누르면 하단 블럭이 연장되면서 리뷰를 작성할 수 있는 박스가 생길거야. 구현해줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **Gemini CLI 사용 프롬프트**:
```
호스트는 게스트가 결제를 완료했을 때, 게스트의 신뢰도를 평가할 수 있는 기능을 만들거야. 1점 매우 신뢰하지 못한다 부터 5점 매우 신뢰한다 까지로 이루어져 있고, 이 점수를 기반으로 게스트의 신뢰도가 낮아질수도, 높아질수도 있어. 1점이면 신뢰도 0.2점, 2점이면 0.1점이 낮아지고, 3점이면 그대로, 4점이면 신뢰도 0.1점, 5점이면 신뢰도 0.2점이 높아져. 구현해줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **Gemini CLI 사용 프롬프트**:
```
게스트는 리뷰 작성까지 모두 했을 때, 호스트의 신뢰도를 평가할 수 있는 기능을 만들거야. 호스트 기능과 동일하게 만들어줘
```
- **결과 및 수정사항**: 구현 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가
...
- **Gemini CLI 사용 프롬프트**:
```
신뢰도 점수 부분이 아얘 잘못되었어. 신뢰도 점수는 0-100 구간이야. 처음 계정을 만들면 신뢰도 점수가 10점이고, 신원 인증을 하면 현재 신뢰도 점수 +20이야. 게스트와 호스트 간 신뢰도 평가를 할 때, 평가 항목이 1-5이고, 이 항목에 따라 신뢰도가 -0.2, -0.1, 0, +0.1, +0.2 가 되는거야.
```
- **결과 및 수정사항**: 개선 성공
- **학습 내용**: 사용자 역할(호스트/게스트)에 따른 기능 추가

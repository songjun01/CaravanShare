// client/src/components/GuestRatingForm.jsx
import React, { useState } from 'react';
import axios from 'axios';
import { useAuth } from '../context/AuthContext';

export default function GuestRatingForm({ reservationId, onRatingSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/v1/ratings/guest`,
        { reservationId, rating },
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      onRatingSubmit();
    } catch (err) {
      console.error('Guest rating submission error:', err);
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 bg-gray-50">
      <h3 className="text-lg font-medium text-gray-900 mb-2">게스트 신뢰도 평가</h3>
